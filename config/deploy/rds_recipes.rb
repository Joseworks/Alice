namespace :rds do
  ...
  set :mysqlcmd, "mysql -h #{rds[:host]} -u #{rds[:master_user]} --password=#{rds[:master_pwd]} -P #{rds[:port]}"
  set :mysqldumpcmd, "mysqldump -h #{rds[:host]} -u #{rds[:master_user]} --password=#{rds[:master_pwd]}"
  ...
  desc <<-DESC
    Drops all reference data from tables used in parts management (includes selections)
  DESC
  task :truncate_shapes do
    load_config
    run %{#{mysqlcmd} #{cfg[:db_name]} -e "truncate placements; truncate shapes; truncate included_shapes; truncate configurations; truncate selections; truncate achievements; truncate achievement_measures;"}
  end
